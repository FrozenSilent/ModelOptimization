...
def count_model_param_flops(model=None, input_res=224, multiply_adds=True, device=torch.device("cuda:0")):
    ...
    if model == None:
        model = torchvision.models.alexnet()
    foo(model)
    input = Variable(torch.rand(3,input_res,input_res).unsqueeze(0).to(device), requires_grad = True)
    out = model(input)
