...
# others
parser.add_argument("--test", action="store_true", dest="test", default=False, help="test model")
parser.add_argument("--prune", action="store_true", dest="prune", default=False, help="prune the model or not")
parser.add_argument("--prune_seq", action="store_true", dest="prune_seq", default=False, help="prune the model for different layers (1-10)")
parser.add_argument('--prune_layers', type=int, default=1, help='number of layers you want to remove')
...
def main(prune_layers=0):
	...
	start_epoch = 1
	if args.prune and os.path.exists(os.path.join(args.save_root, 'model_best.pth.tar')):
		pretrained_checkpoint = torch.load(os.path.join(args.save_root, 'model_best.pth.tar'))
		start_epoch = pretrained_checkpoint['epoch'] + 1
		save_root = args.save_root + '_prune' + str(prune_layers)
	...
	if pretrained_checkpoint is not None:
		load_pretrained_model(snet, pretrained_checkpoint['snet'])
	else:
	...
	# prune the student net
	if args.prune:
		from prune import prune_model
		snet = prune_model(snet, prune_layers, 32, device)

	tnet = define_tsnet(name=args.t_name, num_class=args.num_class, cuda=args.cuda)
	if pretrained_checkpoint is not None:
		load_pretrained_model(tnet, pretrained_checkpoint['tnet'])
	else:
	...
	if args.kd_mode == 'logits':
		...
		s_channels  = snet.get_channel_num()[1:4]
		t_channels  = tnet.get_channel_num()[1:4]
		criterionKD = []
	...
	if args.kd_mode in ['fsp', 'ab'] and not args.prune:
	...
	if args.prune:
		...
		test_top1, test_top5 = test(test_loader, nets, criterions, start_epoch)
	...
	for epoch in range(start_epoch, args.epochs):
		...
		if args.prune:
			params = sum([p.numel() for p in nets['snet'].model.parameters()])
...
def main_test():
	...
	logging.info('----------- Network Initialization --------------')
	if os.path.exists(os.path.join(args.save_root, 'model_best.pth.tar')):
	...
	tnet = define_tsnet(name=args.t_name, num_class=args.num_class, cuda=args.cuda)
	load_pretrained_model(tnet, checkpoint['tnet'])
	tnet.eval()
	for param in tnet.parameters():
		param.requires_grad = False
	# logging.info('Teacher: %s', tnet)
	...
	if args.prune:
		...
		if args.prune_seq:
			...
			# params_after_all = []
			# flops_after_all = []
			snet_copy = copy.deepcopy(snet)
			for i in range(1,11):
				snet = copy.deepcopy(snet_copy)
				snet = prune_model(snet, i, 32, device)
				# params_after_all.append(snet.param_after)
				# flops_after_all.append(snet.flops_after.detach().cpu().numpy())
				...
				print('checkpoint prec@1:%f'%(checkpoint['prec@1']))
				test_top1, test_top5 = test(test_loader, nets, criterions, 0)
				print('rerun prec@1:%f'%(test_top1))
				test_top1_all.append(test_top1)
			print(test_top1_all)
			# print(params_after_all)
			# print(np.array(flops_after_all))
			return None

		else:
			snet = prune_model(snet, args.prune_layers, 32, device)
...
def load_test(prune_layers=1):
	save_root = args.save_root + '_prune' + str(prune_layers)
	if os.path.exists(os.path.join(save_root, 'model_best.pth.tar')):
		checkpoint = torch.load(os.path.join(save_root, 'model_best.pth.tar'))
		print('checkpoint prec@1:%f'%(checkpoint['prec@1']))
		return checkpoint['prec@1']
	else:
		return None
...
def adjust_lr(optimizer, epoch):
	...
	lr_list += [args.lr*scale*scale] * 500
	lr_list += [args.lr*scale*scale*scale] * 500
